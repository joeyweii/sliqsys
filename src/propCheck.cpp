...
double BDDSystem::sparsity(Tensor *tensor) const {
    ...
    double sp = 1 - Cudd_CountMinterm(_ddManager, dd, tensor->_rank) /
                        pow(2, tensor->_rank);
    Cudd_RecursiveDeref(_ddManager, dd);
    return sp;
}
...
bool BDDSystem::eqCheckTwoTensor(Tensor *tensor1, Tensor *tensor2) {
    if ((tensor1->_k % 2) != (tensor2->_k % 2)) increaseTensorKByOne(tensor2);
    ...
    if (tensor1->_r > tensor2->_r) std::swap(tensor1, tensor2);

    for (int i = 0; i < _w; ++i) {
        for (int j = 0; j < tensor1->_r; ++j) {
            if (tensor1->_allBDD[i][j] != tensor2->_allBDD[i][j]) return false;
        }
